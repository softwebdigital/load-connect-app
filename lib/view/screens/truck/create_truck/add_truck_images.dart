...
class AddTruckImagesScreen extends StatelessWidget {
  const AddTruckImagesScreen({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            if (provider.truckImages.isNotEmpty)
              GridView(
                shrinkWrap: true,
                ...
                children: [
                  for (var image in provider.truckImages)
                    Container(
                      ...
                      child: GestureDetector(
                        onTap: () {
                          // provider.truckImages.removeImage(
                          //   controller.pickedImages.indexOf(image),
                          // );
                          provider.removeImage(provider.truckImages.indexOf(image));
                        },
            ...
            // Con
            SizeMargin.size(
              height: provider.truckImages.isEmpty ? 50.0 : 24.0,
            ),
            TextButton.icon(
              onPressed: () {
                provider.selectTruckImages(context);
              },
              icon: const Icon(
            ...
            CustomRaisedButton(
              ...
              onPressed: () {
                // Get.toNamed(Routes.addTruckDocuments);
                provider.next();
              },
            ),
            SizeMargin.size(height: 24.0),
